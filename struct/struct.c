...
void imprimirPessoa(Pessoa p) { // Criação de um procedimento (função que não retorna nada)
  printf("Nome: %s\n", p.nome);
  printf("Idade: %d\n", p.idade);
  printf("Sexo: %c\n", p.sexo);
...
//   pessoa = lerPessoa(); // A variável do tipo Pessoa recebe o retorno da função lerPessoa(). lerPessoa é do tipo Pessoa(struct)
